...
Andre semi-modulære PLS-er bruker IO-kort for å utvide grunnmodulen. Koyo DirectLogic DL06 er et eksempel på denne type PLS. Bildet viser DL06 som det settes et kort for termoelementer i en av de 4 ledige plassene.\index{Koyo DL06 PLC}
...
IO-tilkoblinger for en PLS kommer i to ulike kategorier \textit{digitale} og \textit{ analoge} disse beskrives i de neste kaptittelene. 


\filbreak
\subsection{Digitale IO-er}

Et digitalt singal for en PLS, er et signal som kan ha to tilstander \textit{av} og \textit{på}. Disse bruker datatypen BOOL i internt i PLS-en. Trykkbrytere, temperaturbrytere, trykknappbrytere, endebrytere og nærhetssensorer er eksempler på digitale signalkilder for en PLS. For at PLS-en skal lese signaler fra denne type sensorer må den ha en digitale innganger. Digitale innganger på en PLS vil ha en lysdiode som indikerer om en inngang er aktivert eller ikke. Det vil også være en annen LED som er en del av en optokobler. Optokobleren lager et galvansik skille mellom sensorkretsen og mikroprosessorkretsen inne i PLS-en. Et slikt system gjør at en PLS er mindre følsom fra spenningstransienter. Optokobleren aktiver et bit inn minnet til PLS-en. Dette minnet brukes av PLS programmet til å avgjøre om en sensor er aktivert eller ikke. \index{DIO, Digital IO} \index{Bit} 
...
Et viktig konsept å beherske når en jobber med digitale inn- og utganger er forskjellen mellom \textit{current-sourcing} og \textit{current-sinking}. Begrepene "sourcing" og "sinking" refererer til retningen til strømmen inn eller ut av terminalen for styresignal. En enhet som sender strøm ut av styreterminalen sies å \textit{sourcing}, mens utstrm som tar imont strøm sies å være \textit{sinking}.\index{Sourcing current}  \index{Sinking current}  \index{Current sourcing}  \index{Current sinking}

\filbreak

Tegningen nedenfor illustrerer PLS utgang som er current \textit{sourcing} for en lampe som er current\textit{sinking}.
...
Om det tilkoblede utstyret polaritetsuavhengig, er det like gylding om en har \textit{sourcing} eller \textit{sinking} DI-modul. Bildet viser et eksempel med en endebryter. 
...
Legg merke til polariteten på fellesterminalen på begge moduelene. Modulen som er \textit{sinking} har fellesterminalen koblet til minus, mens modulen som  er \textit{sourcing} er koblet til +. 


\filbreak

Noen digitale sensorer \textit{har} en bestempt polaritet, f.eks. nærhetssensorer med transistorutgang. En \textit{sourcing} nærhetssensor kan bare kobles til en \textit{sinking} PLS inngang, og visa versa.\index{Sourcing current}  \index{Sinking current}  \index{Current sourcing}  \index{Current sinking}

$$\includegraphics{plc_012.eps}$$

I alle tilfellene sender enheten som er \textit{sourcing} ut strøm fra signalterminalen til \textit{sinking} enheten som mottar signal på sin signalterminal. 
...
Det kan se ut som alle PLS-er har sin egen standard for programmering, men det finnes en international standard for PLS progarmmering som de fleste PLS produsenter i det minste forsøker å følge. Dette er IEC 61131-3, som vi vil bruke her.\index{IEC standard 61131-3 (PLC programming languages)}
...
En grunnleggende regel en må tenke på når en leser et ladder program er at \textbf{hver kontakt i programmet aktiveres ved at den leser "1" i det tilhørende minnebit,  og at den deaktiveres når den leser "0" i det tilhørdne minnebit.}. For en NO kontakt vil det si at den er lukket nå tilhørdne minnebit er "1" For en NO kontakt vil det si at den er lukket når tilhørende minnebit er "0". Om et bit har tilstanden "0"  vil det si at tilhørdne kontakt er i sin normale posisjon, mens en bitstatus på "1" vil få kontakten til å aktivere og gå i sin aktive tilstand. \index{PLS Normal status på brytere}  

En annen viktig regel når en leser PLS programmer er at programmeringsgrensesnittet tilbyr \textit{farge merkeing} for å markere virtuel status av hvert program element. \textbf{En farget kontakt(\colorbox{yellow!50}{-$|$ $|$-}) er \textit{lukket}, mens en ikke farget (-$|$ $|$-) er \textit{åpen}}. Om  bryteren har en skråstrek (-$|/|$-) eller ikke (-$|\:|$-) marker hvilken normale status brytern har, så marker farge om den er lukket eller ikke.
...
Akkurat som en trykkbryter aktiveres av høyt trykk, og en nivåbryter aktiveres av høyt nivå, og en temperaturbyter aktiveres av høy temperatur, så vil en PLS sine virtuelle brytere aktiveres av et høyt \textit{bit} status. Med en bryte sin \textit{aktiverte} tilstand mener vi det motsatte av den \textit{normale} tilstand.


\filbreak

Illustrasjonen nedenfor viser en kompakt PLS med to av de digitale inngangene aktivert, noe som gjør at de tilhørende bit i inngangsregisteret er høye (TRUE eller "1"). De kontaktene som er farget i PLS programmeringsverktøyet viser virtuelle kontakter som vil sende \textit{signalet} videre ved de statuser som vises i  inngangsregisteret.

$$\includegraphics{plc_064.eps}$$

\textbf{Husk} at en farget kontakt er en \textit{lukket} kontakt. De kontaktene som er farget er enten NC kontakter med "0" bit status, eller NO kontakter med "1" bit status. Det er kombinasjonen av bit status og type kontakt (NO/NC) som avgjør om den virtuelle kontakten vil  være åpen (ikke farget) eller lukket (farget). 

\filbreak

Min erfaring som lærer er at hovedproblemet elever har med å forstå PLS ladderprogrammer, er at de prøver å simplifisere ved å relatere vituelle kontakter i pls programmet med kontakter ute i anlegget. Det som skjer er at kontakter i anlegget sender spenning til inngange \textit{som igjen \underbar{styrer}} status til den virtuelle kontakten i PLS programmet. Typiske feil som elever gjøre er:

\begin{itemize}
	\item Elever tror kontakten i PLS programmet må være av samme type (NO/NC) som tilhørende bryter i anlegget
	\item Elever tror at fargemerkingen av en kontakt er det samme som status på tilhørende inngang. 
	\item Elever tror en lukket kontakt i anlegget må resultere i en lukket kontakt i PLS programmet.
\end{itemize}
...
\item Normalt
\end{itemize}
%Dette bildet må genereres på nytt
%\includegraphics[width=0.5\textwidth]{\string"SFC presentasjon/15\string".eps}
...
% TODU list
% denne må utvidet med entry og exit acitons
...
\end{itemize}
%\includegraphics[width=0.7\textwidth]{SFC_ClampAndDrill}
...
\includegraphics[width=0.7\textwidth]{SFC_stoppet_maskin}
\vskip 2.5pt 
...
\includegraphics[width=0.7\textwidth]{SFC_EnkeltTrefikkLys}
\vskip 2.5pt 
\vskip\eject
\textbf{Eksempel}
\vskip 2.5pt 
...
\includegraphics[width=1\textwidth]{SFC_KombinasjonavSpråk}
\vskip 2.5pt 
...
%\includegraphics[width=0.5\textwidth]{\string"SFC presentasjon/47\string".eps}\includegraphics[width=0.5\textwidth]{\string"SFC presentasjon/50\string".eps}

Eksempel på sekvensstyring:

\begin{center}
%\includegraphics[scale=0.9]{Veiarbeid}
\par\end{center}

Funksjonstabell for lysanlegget

\begin{center}
%\includegraphics[width=17cm]{Sekvenser}
\par\end{center}
