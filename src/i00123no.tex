...
Oppgaven er å bygge, dokumentere og feilsøke et nivåmålesystem for væske som består av a pneumatic $\Delta$P transmitter connected to a local ``receiver gauge'' pneumatic indicator as well as to a pressure transmitter to relay that same level measurement to a remotely-located electronic indicator.  Water held in a vertical tube is the suggested process variable to measure, but other liquid level variables are open for consideration, though.  Alternatives to the standard level-measurement lab are authorized by instructor permission only.

En del av denne labøvelsen er bruk av {\it væskemanometer} som standardinstrument for trykkverifisering.  En annen del er korrekt identifikasjon av vanlige rør- og tubefittings.  En 3-ventils eller 5-ventils manifold må være montert på senderen for isolering og testing.
...
%
% Another row
Ranging av sløyfe ($\pm$ 1\% av spennnøyaktighet) & mestring & & & & & -- -- -- -- \cr
%
\noalign{\hrule}
%
% Another row
Bruk av manometer & mestring & & & & & -- -- -- -- \cr
%
\noalign{\hrule}
%
% Another row
...
%
% Another row
Identifikasjon av rør- og tubefittings & mestring & -- & -- & -- & -- & \cr
%
\noalign{\hrule}
%
% Another row
...
Den eneste ``proporsjonale'' vurderingen i denne aktiviteten er labspørsmålene, som besvares individuelt av hver student.  A listing of potential lab questions are shown at the end of this worksheet question.  The lab questions are intended to guide your labwork as much as they are intended to measure your comprehension, and as such the instructor may ask these questions of your team day by day, rather than all at once (on a single day).

\vskip 10pt

{\bf Det er avgjørende at teamet planlegger på forhånd hva som skal gjennomføres hver dag.  A short (10 minute) team meeting at the beginning of each lab session is a good way to do this, reviewing what's already been done, what's left to do, and what assessments you should be ready for.  There is a lot of work involved with building, documenting, and troubleshooting these working instrument systems!}

Når du og teamet arbeider med systemet, vil dere uunngåelig møte problemer.  Dere bør alltid forsøke å løse problemene som team før dere ber instruktøren om hjelp.  Hvis dere fortsatt trenger hjelp, skriv teamfargen på labtavlen sammen med en kort beskrivelse av hva dere trenger hjelp til.  The instructor will meet with each team in order they appear on the whiteboard to address these problems.
...
This exercise tests your ability to navigate a ``smart'' instrument's parameters using a communicator as well as properly interpret terminal connections on a feltinstrument for signal and power.
...
\noindent
{\bf Transmitter range} (instruktør velger): \hskip 20pt LRV = \underbar{\hskip 50pt} \hskip 20pt URV = \underbar{\hskip 50pt}

\vskip 10pt

\noindent
{\bf Meter options} (instruktør velger): \hskip 20pt \underbar{\hskip 20pt} Voltmeter (1-5 VDC) \hskip 10pt {\it or} \hskip 10pt \underbar{\hskip 20pt} Amperemeter (4-20 mA)

\vskip 10pt

\noindent
{\bf Signalet øker med...} (instruktør velger): \hskip 20pt \underbar{\hskip 20pt} Positivt trykk \hskip 10pt {\it or} \hskip 10pt \underbar{\hskip 20pt} Vakuum (sug)

\vskip 10pt

\vfil

Studiehenvisning: the ``Analog Electronic Instrumentation'' chapter of {\it Lessons In Industrial Instrumentation}, particularly the sections on loop-powered transmitters and current loop troubleshooting.  Also, the ``Basic Concept of HART'' subsection of the ``The HART Digital/Analog Hybrid Standard'' section of the ``Digital Data Acquisition and Networks'' chapter of the same book.
...
After getting your prototypeskisse approved by the instructor, you are cleared to begin building your system.  Transmitters attach to 2-inch pipes using special brackets and U-bolts.  These brackets and U-bolts are located along with the transmitters in the instrument storage area.  Feel free to use 1/4 inch plastic tubing for all the pneumatic signal connections, and be sure not to exceed the rated supply pressure (as documented in the transmitter's manual).
...
\begin{itemize}
\item{} Neglecting to consult the manufacturer's documentation for feltinstruments (e.g. how to connect pneumatic signal lines, how to calibrate them).
\item{} Mounting the feltinstrument(s) in awkward positions, making it difficult to reach tube connectors or to remove covers when installed.
...
Hver student må tegne sitt eget {\it sløyfediagram} for teamets system, i tråd med ISA-konvensjoner.  Sample sløyfediagrams are shown in the next question in this worksheet.  These sløyfediagrams must be {\it comprehensive} and {\it detailed}, showing every tube connection, every tube, range points, etc.  The principle to keep in mind here is to make the sløyfediagram so complete and unambiguous that anyone can follow it to see what connects to what, even someone unfamiliar with industrial instrumentation.  In industry, loops are often constructed by contract personnel with limited understanding of how the system is supposed to function.  The sløyfediagrams they follow must be so complete that they will be able to connect everything properly without necessarily understanding how it is supposed to work.

Every instrument and every signal tube and signal cable in your loop needs to be properly labeled with an ISA-standard tag number.  An easy way to do this is to wrap a short piece of masking tape around each tube (and placed on each instrument) then writing on that masking tape with a permanent marker.  Although no industry standard exists for labeling signal cables and tubes, a good recommendation is to label each tube with the tag number of the feltinstrument it goes to.  Thus, every length of tube in a pressure transmitter loop should be labeled ``PT-$x$'' (where ``$x$'' is the loop number), every flow control valve should be labeled ``FV-$x$'', etc.  Remember that the entire loop is defined by the process variable it measures: if the PV is {\it temperature} then the transmitter with be a {\it T}T, the control valve will be a {\it T}V, the controller with be a {\it T}C, etc.

Please note that this level-measurement system is part pneumatic and part electronic, and that your sløyfediagram must show {\it all} of these details.

Når hele teamet er ferdig med individuelle sløyfediagram, tilkall instruktøren for inspeksjon av sløyfen.  Here, the instructor will have students take turns going through the entire loop, with the other students checking their diagrams for errors and omissions along the way.  During this time the instructor will also inspect the quality of the installation, identifying problems such as frayed wires, improperly crimped terminals, poor cable routing, missing labels, lack of wire duct covers, etc.  The team must correct all identified errors in order to receive credit for their system.  

Etter bestått inspeksjon skal hvert teammedlem legge sløyfediagrammet i diagramholderen midt i laben bak hovedpanelet.  When it comes time to troubleshoot another team's system, this is where you will go to find a sløyfediagram for that system!
...
\begin{itemize}
\item{} Forgetting to label all signal tubes (see example sløyfediagrams).
\item{} Forgetting to label all feltinstruments with their own tag names (e.g. LT-83).
\item{} Forgetting to put your name on the sløyfediagram!
\item{} Basing your diagram off of a teammedlem's diagram, rather than closely inspecting the system for yourself.
\item{} Not placing loop sheet instruments in the correct orientation (feltinstruments on the left, kontrollrom instruments on the right).
\end{itemize}

\vskip 10pt

{\bf Creating and inspecting accurate sløyfediagrams should take no more than one full lab session (3 hours) if the team is working efficiently!}
...
Når teamets sender er ferdig kalibrert, sett på en kalibreringsetikett med område og kalibreringsdato.  A set of calibration tags are shown here which you may cut out and tape to the transmitter after completing your calibration:
...
\noindent
{\bf Labøvelse -- identifikasjon av rør- og tubefittings}

\vskip 5pt

En del av denne labøvelsen er å identifisere følgende typer rør- og instrumenttubefittings fra hukommelsen (uten bruk av bildeoversikt).  Note that synonyms are separated by slash marks (e.g. ``street/run''):
...
The most challenging aspect of this lab exercise is {\it troubleshooting}, where you demonstrate your ability to logically isolate a problem in the system.  All troubleshooting is done on an individual basis (no team credit!), and must be done {\it on a system you did not help build}, so that you must rely on sløyfediagrams to find your way around the system instead of from your own memory of building it.
...
I strongly recommend checking off students' sløyfediagrams while you inspect their loop (checking for secure wiring, proper tubing, good conduit installation, etc.) with them.  Have all team members take you on a ``tour'' of their completed loop, with each team member explaining a different portion of the loop you select while using their own sløyfediagram as a guide.  While a student is explaining their section of the loop, you can check the other students' sløyfediagrams for accuracy.  This not only saves time by consolidating the tasks of loop inspection and sløyfediagram verification, but it also ensures students can actually relate their sløyfediagrams to the loop they have built and articulate that understanding to you.
...
\item{$(4)$} Suppose a pneumatic level transmitter senses liquid level by that liquid's hydrostatic pressure, and reports the level measurement to a 3-15 PSI ``receiver gauge'' located in the kontrollrom.  One day the operator notices the receiver gauge registers well over 100\% level but knows the vessel is only about 40\% full by visual inspection.  A technician decides to test the transmitter by pulling its flapper (baffle) away from its nozzle while asking the operator to view the receiver gauge's indication.  Is this a useful diagnostic test?  Explain why or why not.
\end{itemize}
...
\item{$(4)$} Suppose a pneumatic level transmitter senses liquid level by that liquid's hydrostatic pressure, and reports the level measurement to a 3-15 PSI ``receiver gauge'' located in the kontrollrom.  One day the operator notices the receiver gauge registers well below 0\% level but knows the vessel is over half-full by visual inspection.  A technician decides to test the transmitter by pulling its flapper (baffle) away from its nozzle while asking the operator to view the receiver gauge's indication.  Is this a useful diagnostic test?  Explain why or why not.
\end{itemize}
