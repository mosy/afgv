...
A motor controlled by a PLC refuses to start when the ``Start'' pushbutton switch is pressed.  A fellow instrument technician reports seeing the LED indicator for channel X2 on the input card illuminated while the operator was pressing the ``Stop'' pushbutton, but does not report on the illumination status of any other LEDs on the PLC nor does he notice what happens when the operator releases the pushbutton.  Shown here is a partial wiring diagram and offline PLC program display for the system:

$$\includegraphics[width=15.5cm]{i02184x01.eps}$$
...
%
% Another row
Bit {\tt X2} forced to a ``1'' state in the PLC &  &  \cr
%
\noalign{\hrule}
%
% Another row
