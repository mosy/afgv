...
$$\includegraphics[width=15.5cm]{i03308x01.eps}$$
...
\item{} Re-draw the output trend if the PV had only deviated by 5\% from the setpoint rather than a full 10\% (for the same duration in time).
\item{} Re-draw the output trend if this controller implemented a full PID algorithm.
\end{itemize}

\underbar{file i03308}
%(END_QUESTION)
...
This is a {\it direct-acting}, {\it integral-only} controller.
...
The process trend shown below reveals a controller's response to the process variable signal and the setpoint.  Based on what you see in this trend, determine whether the controller is direct or reverse acting, and also whether it implements a P-only, I-only, P+I, or P+D control algorithm.

$$\includegraphics[width=15.5cm]{i03308x02.eps}$$
