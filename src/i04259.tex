...
In preparation to disconnect and remove an electric motor for rebuild, an electrician shuts off the circuit breaker feeding the motor control circuit, then places a lock and an informational tag on that breaker so that no one turns it back on before she is done with the job.

The next step is to confirm the absence of dangerous voltage on the motor conductors before physically touching any of them.  This confirmation, of course, is done with a voltmeter, and we all know that voltage is measured {\it between two points}.  The question now is, how many different combinations of points must the electrician measure between using her voltmeter to ensure there is {\it no} hazardous voltage present?

$$\includegraphics[width=15.5cm]{i04259x01.eps}$$

List all possible pairs of points the electrician must check for voltage between.  Don't forget to include earth ground as one of those points, in addition to T1, T2, and T3!  
